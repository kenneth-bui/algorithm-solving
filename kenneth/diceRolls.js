 ...
 * Problem description:
    ...
    Write a function: function solution(A, F, M);
...
/**
 * Return an array containing possible results of the missed rolls
 * @param {Array} A
 * @param {Number} F
 * @param {Number} M
 */
function Solution(A, F, M) {
  ...
  let sumF = (A.length + F) * M - sumA;
  return sumF < 0 || sumF > F * 6 || sumF < F? [0] : fillDices(sumF, F);
}

/**
 * Return an array having length = n and sum of array elements = sum
 * @param {Number} sum Remaining sum of missing dice rolls
 * @param {Number} n Number of missing dice rolls
 * @returns
 */
const fillDices = (sum, n) => {
...
console.log(Solution([1, 1, 1, 1], 2, 2));
console.log(Solution([1, 1, 1, 1], 8, 1));
console.log(Solution([2, 1, 1, 1], 8, 1));
